...
int main(int, char**)
{
	...
	if (kernel==NULL){
     ...
     frame2filter_buf = clCreateBuffer(context, CL_MEM_READ_ONLY, S.height*S.width *sizeof(int), NULL, &status);
     checkError(status, "Failed to create buffer for input frame");

     output_buf = clCreateBuffer(context, CL_MEM_WRITE_ONLY, S.height*S.width *sizeof(int), NULL, &status);
	...
	int* output = (int*) malloc(sizeof(int)*S.width*S.height);
	// Set kernel arguments for first gaussian blur

	
    	status = clEnqueueWriteBuffer(queue, frame2filter_buf, CL_FALSE, 0, S.height*S.width* sizeof(int), source_image, 0, NULL, &write_event[0]);
	...
	*/
	...
	status = clEnqueueReadBuffer(queue, output_buf, CL_TRUE, 0, S.height*S.width* sizeof(int), output, 1, &kernel_event, &finish_event);
	
	status = clEnqueueWriteBuffer(queue, frame2filter_buf, CL_FALSE, 0, S.height*S.width* sizeof(int), output, 1, &finish_event, &write_event[0]);
	checkError(status, "Failed to transfer input image");
